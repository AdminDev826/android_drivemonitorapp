...
import liveo.adapter.NavigationLiveoItemAdapter;
...
public class NavigationLiveoList {
    ...
    public static List<NavigationLiveoItemAdapter> getNavigationAdapter(Context context, Navigation navigation) {
        ...
        for (int i = 0; i < navigation.nameItem.size(); i++) {
            ...
            if (isHeader && icon > 0){
                throw new RuntimeException(context.getString(br.liveo.navigationliveo.R.string.value_icon_should_be_0));
            }

            if (!isHeader) {
                if (title == null) {
                    throw new RuntimeException(context.getString(br.liveo.navigationliveo.R.string.enter_item_name_position) + i);
                }

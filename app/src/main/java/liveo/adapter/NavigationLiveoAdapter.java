...
public class NavigationLiveoAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (holder.title != null){
            ...
            if (!item.isHeader && item.checked && item.colorSelected > 0)
                holder.title.setTextColor(ContextCompat.getColor(mcontext, item.colorSelected));
            else if (mColorDefault != 0)
                holder.title.setTextColor(ContextCompat.getColor(mcontext, mColorDefault));
            else if (mColorName > 0)
                holder.title.setTextColor(ContextCompat.getColor(mcontext, mColorName));
            else if (item.isHeader && mColorSubHeader > 0)
                holder.title.setTextColor(ContextCompat.getColor(mcontext, mColorSubHeader));
            else holder.title.setTextColor(ContextCompat.getColor(mcontext, R.color.nliveo_black));
		}

		if (holder.counter != null) {
			if (item.counter >= 1) {
                holder.counter.setBackgroundResource(R.drawable.showonline_status);
                setAlpha(holder.counter, (item.checked ? 1f : 0.87f));
				holder.counter.setVisibility(View.VISIBLE);
//                holder.counter.setText((item.counter > 99) ? "99+" : item.counter + "");

                if (!item.isHeader && item.checked && item.colorSelected > 0)
                    holder.counter.setTextColor(ContextCompat.getColor(mcontext, item.colorSelected));
                else if (mColorDefault != 0)
                    holder.counter.setTextColor(ContextCompat.getColor(mcontext, mColorDefault));
                else if (mColorCounter > 0)
                    holder.counter.setTextColor(ContextCompat.getColor(mcontext, mColorCounter));
                else
                    holder.counter.setTextColor(ContextCompat.getColor(mcontext, R.color.nliveo_black));

            } else {
                holder.counter.setBackgroundResource(R.drawable.showoffline_status);
